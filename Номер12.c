...
int max_row(int N, int M){
    ...
    S=0;
    MaxSum=0;
     a=(float **) malloc(N*sizeof(float*));  // выделение пам€ти
    if (a == NULL)
        {
    ...
    for  (i=0; i<N; i++){
        S=0;
        for(j=0; j<M; j++) {    // последовательно вычисл€ютс€ суммы строк
            scanf("%f", &a[i][j]);
            S+=a[i][j];
        }
        if (S > MaxSum) {  // находим номер строки  с максимальной суммой
            MaxSum=S;
            k=i;
        }
    }
